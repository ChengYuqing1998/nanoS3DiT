...
本项目的模型架构和训练方式主要参考 **Z-Image**，同时有意保持实现精简：

- 文本 token 与图像 latent token 拼接为单一序列；
- 两种模态由共享的 Transformer block 联合处理；
- timestep 通过 adaptive normalization 注入；
- 使用 flow matching 目标进行训练；
- 使用 Z-Image VAE 编码和解码图像 latent。

本项目不是 Z-Image 官方实现，也不是对其完整模型的精确复现。

**Z-Image 链接：** [GitHub](https://github.com/Tongyi-MAI/Z-Image) | [Hugging Face](https://huggingface.co/Tongyi-MAI/Z-Image) | [论文](https://arxiv.org/abs/2511.22699)

## 预期结果

正常完成 overfit 训练后，推理结果应与下面的示例类似。左侧小图是 prompt `J` 对应的 28x28 EMNIST 原始训练样本；右侧大图是使用 step 8000 checkpoint 和 seed 42 生成的 128x128 推理结果。

| 原始训练样本 | 推理生成结果 |
|:---:|:---:|
| <img src="assets/examples/emnist-j-reference.jpg" alt="EMNIST 字母 J 原始训练样本" width="112"> | <img src="assets/examples/emnist-j-step8000-seed42.png" alt="step 8000 seed 42 生成的字母 J" width="256"> |
...
configs/nano-s3dit-overfit-178m.toml         177,577,248 参数（约 178M），训练 2000 step
